...
int simulador_adivinar_nivel_pokemon(simulador_t* simulador, void* datos){ 
    ...
    printf("Nombre del pokemon a adivinar: %s\n",simulador->info_pokemon->nombre_pokemon);
    ...
    if(intento->es_correcto){
        simulador->estadisticas->puntos += dificultad_actual->datos_dificultad->calcular_puntaje(simulador->cantidad_intentos);
